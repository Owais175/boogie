        ...
        <div class="col-md-12">
            <div class="form-group">
                ...
                <input class="form-control dropify" name="image" type="file" id="image"
                    {{ $page->image != '' ? "data-default-file = /$page->image" : '' }}
                ...
                <div class="form-group">
                    ...
                    @elseif($section->type == 'video')
                        <img alt="" class="img-responsive" id="banner1" src="{{ asset($section->value) }}"
                            style=" width: 30%; ">
                        <input type="file" name="{{ $section->slug }}" class="dropify"
                            {{ $section->value != '' ? "data-default-file = /$section->value" : '' }}
                            {{ $section->value == '' ? 'required' : '' }} value="{{ $section->value }}">
                    @else($section->type == 'text')
                        <input type="text" name="{{ $section->slug }}" value="{{ $section->value }}"
                            class="form-control">
                    @endif
                </div>
            </div>
